...
"""wotao yin mod 0.2, 2013-05-07."""

# Copyright (C) 2007-2012 Jacob Mattingley (jacobm@stanford.edu).
#
# Modified by Wotao Yin to support
#   - compatibility with a new CSS style
#   - add "-mathjax" option
#   - add "backlink" option for the modify line
#   - use of latest Google Analytics script
#   - add an option to place image in the center using {img_center} instead of {img_left}
#   - add an option to place two images in the center using {two_img_center}
#
...
# # wotao: to automatically insert argument "l1_uniquness" for debugger
# if __name__ == '__main__':
#   numArgs = len(sys.argv)
#   if numArgs == 1:
#     myArg = "l1_uniqueness" # raw_input('please supply missing argument: ')
#   else:
#     myArg = sys.argv[1]
#   print 'argument : %s' % myArg
#   sys.argv = [sys.argv[0], myArg]
...
class controlstruct(object):
  def __init__(self, infile, outfile=None, conf=None, inname=None, eqs=True,
    ...
    self.otherfiles = []
    self.eqs = eqs
    self.mathjax = False    # wotao: switch for mathjax
    self.eqdir = eqdir
    self.eqdpi = eqdpi
...
def standardconf():
  ...
  [mathjax]
  <!-- wotao: include script for mathjax -->
  ...
  [lastupdated]
  generated |, by <a href="http://jemdoc.jaboc.net/">jemdoc</a>
  # wotao: text updated
...
def insertmenuitems(f, mname, current, prefix):
  m = open(mname, 'rb')
  mc_start = 0  # wotao: menu category not started
  while pc(controlstruct(m)) != '':
    l = readnoncomment(m)
    l = l.strip()
    if l == '':
      if mc_start == 0:     # wotao: if menu category not started
        continue
      else:                 # wotao: if started, put in [menucategoryend]
        mc_start = 0 # finish menu category
        out(f.outf, f.conf['menucategoryend'])
        continue
    ...
    if r: # then we have a menu item.
      ...
      out(f.outf, f.conf['menucategorystart'])  # wotao: insert [menucategorystart]
      mc_start = 1
...
def replaceequations(b, f):
  ...
  for (r, wl) in rs:
    ...
    while m:
      ...
      # wotao: if mathjax is turned on, skip latex typesetting
      if f.mathjax:
        eq = re.sub("\\\\", 'jemLITerl33talWy1BS', eq)  # wotao: replace "\" by "jemLITerl33talWy1BS", which will be replaced back
        if wl:
          b = b[:m.start()] + '\\\(%s\\\)' % eq + b[m.end():]  # wotao: display equation
        else:
          b = b[:m.start()] + '\$%s\$' % eq + b[m.end():]    # wotao: inline equation
        m = r.search(b, m.start())
        continue  # wotao: skip the rest part, which typesets latex
...
def br(b, f, tableblock=False):
  ...
  b = re.sub('jemLITerl33talWy1BS', r'\\', b)   # wotao: replace back "\" in latex for mathjax
...
def geneq(f, eq, dpi, wl, outname):
  ...
  try:
    ...
    # wotao: switch "--freetype0" is removed to prevent error under Windows
    dvicmd = 'dvipng -Q 9 -z 3 --depth -q -T tight -D %i -bg Transparent -o %s %s' % (dpi, eqname, dvifile)
...
def colonlist(f):
  ...
  while pc(f) == ':':
    ...
    hb(f.outf, '<dt class="paper-entry">|</dt>\n', br(defpart, f))  # wotao: "paper-entry" style added
    hb(f.outf, '<dd><p>|</p></dd>\n', br(rest, f))
...
def procfile(f):
  ...
  showbacklink = False  # wotao: switch for a 'back' link
  showfooter = True
  ...
  while pc(f, False) == '#':
    ...
    if l.startswith('# jemdoc:'):
      ...
      for b in a:
        ...
        # wotao: add an option to include a 'back' link
        elif b.startswith('backlink'):
          showbacklink = True
        ...
        elif b.startswith('mathjax'):   # wotao: process modeline switch "mathjax"
          f.mathjax = True
  ...
  # wotao: insert mathjax script between <head> and </head>
  if f.mathjax:
    out(f.outf, f.conf['mathjax'])
...
# wotao: replace analytics between <head> and </head>
  if f.analytics:
  ...
  while 1: # wait for EOF.
    ...
    # look for blocks.
      ...
      if infoblock:
        ...
        # wotao: added to support images place in line center
          ...
          out(f.outf, '<table class="imgtable" align="center"><tr><td>\n')
          if g[6]:
            out(f.outf, '<a href="%s">' % g[6])
          out(f.outf, '<img src="%s"' % g[2])
          out(f.outf, ' alt="%s"' % g[3])
        ...
        # wotao: added to support multiple images place in line center
        elif len(g) >= 4 and g[1] == 'two_img_center':
          # handles
          # {}{two_img_center}{source 1}{alttext 1}{width 1}{height 1}{linktarget 1}{source 2}{alttext 2}{width 2}{height 2}{linktarget 2}.
          ...
          out(f.outf, '<table class="imgtable" align="center"><tr>\n')
          ...
          if g[11]:
            out(f.outf, '<a href="%s">' % g[11])
          out(f.outf, '<img src="%s"' % g[7])
          out(f.outf, ' alt="%s"' % g[8])
          if g[9]:
            out(f.outf, ' width="%s"' % g[9])
          if g[10]:
            out(f.outf, ' height="%s"' % g[10])
          out(f.outf, ' />')
          if g[11]:
            out(f.outf, '</a>')
          out(f.outf, '&nbsp;</td>\n<td>')
...
# # wotao: will show footer later after the "wrapper" div
...
#    out(f.outf, f.conf['footerend'])
  ...
  # wotao: add a 'back' link
  if showbacklink:
    out(f.outf, f.conf['backlink'])
  ...
  # wotao: showfooter is moved here
  if showfooter and (showlastupdated or showsourcelink):
    out(f.outf, f.conf['footerstart'])
    if showlastupdated:
      if showlastupdatedtime:

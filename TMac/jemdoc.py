...
# Modified by Yangyang Yin on 2012-11-27 for a new style and mathjax
#
...
# # Yangyang: to automatically insert argument "l1_uniquness" for debugger
...
class controlstruct(object):
  def __init__(self, infile, outfile=None, conf=None, inname=None, eqs=True,
    ...
    self.otherfiles = []
    self.eqs = eqs
    self.mathjax = False    # Yangyang: switch for mathjax
    self.eqdir = eqdir
    self.eqdpi = eqdpi
...
def standardconf():
  ...
  [mathjax]
  <!-- Yangyang: include script for mathjax -->
  ...
  [lastupdated]
  generated |, by <a href="http://jemdoc.jaboc.net/">jemdoc</a> (modified)
  # Yangyang: text updated
...
def insertmenuitems(f, mname, current, prefix):
  m = open(mname, 'rb')
  mc_start = 0  # Yangyang: menu category not started
  while pc(controlstruct(m)) != '':
    l = readnoncomment(m)
    l = l.strip()
    if l == '':
      if mc_start == 0:     # Yangyang: if menu category not started
        continue
      else:                 # Yangyang: if started, put in [menucategoryend]
    ...
    if r: # then we have a menu item.
      ...
      out(f.outf, f.conf['menucategorystart'])  # Yangyang: insert [menucategorystart]
      mc_start = 1
...
def replaceequations(b, f):
  ...
  for (r, wl) in rs:
    ...
    while m:
      ...
      # Yangyang: if mathjax is turned on, skip latex typesetting
      if f.mathjax:
        eq = re.sub("\\\\", 'jemLITerl33talWy1BS', eq)  # Yangyang: replace "\" by "jemLITerl33talWy1BS", which will be replaced back
        if wl:
          b = b[:m.start()] + '\\\(%s\\\)' % eq + b[m.end():]  # Yangyang: display equation
        else:
          b = b[:m.start()] + '\$%s\$' % eq + b[m.end():]    # Yangyang: inline equation
        m = r.search(b, m.start())
        continue  # Yangyang: skip the rest part, which typesets latex
...
def br(b, f, tableblock=False):
  ...
  b = re.sub('jemLITerl33talWy1BS', r'\\', b)   # Yangyang: replace back "\" in latex for mathjax
...
def geneq(f, eq, dpi, wl, outname):
  ...
  try:
    ...
    # Yangyang: switch "--freetype0" is removed to prevent error under Windows
...
def colonlist(f):
  ...
  while pc(f) == ':':
    ...
    hb(f.outf, '<dt class="paper-entry">|</dt>\n', br(defpart, f))  # Yangyang: "paper-entry" style added
    hb(f.outf, '<dd><p>|</p></dd>\n', br(rest, f))
...
def procfile(f):
  ...
  while pc(f, False) == '#':
    ...
    if l.startswith('# jemdoc:'):
      ...
      for b in a:
        ...
        elif b.startswith('mathjax'):   # Yangyang: process modeline switch "mathjax"
          f.mathjax = True
  ...
  # Yangyang: insert mathjax script between <head> and </head>
  if f.mathjax:
  ...
  if not nodefaulticon:
	out(f.outf, f.conf['defaulticon'])
...
# Yangyang: replace analytics between <head> and </head>
  if f.analytics:
  ...
  while 1: # wait for EOF.
    ...
    # look for blocks.
      ...
      if infoblock:
        ...
        # Yangyang: added to support images place in line center
...
# # Yangyang: will show footer later after the "wrapper" div
...
#    out(f.outf, f.conf['footerend'])
  ...
  # Yangyang: showfooter is moved here
  if showfooter and (showlastupdated or showsourcelink):
    out(f.outf, f.conf['footerstart'])
    if showlastupdated:
      if showlastupdatedtime:
